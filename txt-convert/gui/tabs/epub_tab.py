...
class EpubTab(BaseTab):
    ...
    def __init__(self, parent, app):
        ...
        if not EPUB_SUPPORT:
            ctk.CTkLabel(self.frame,
                         text="⚠️ 未检测到 ebooklib，请运行: pip install ebooklib pillow requests",
                         text_color='#fca5a5', font=get_ui_font(9)).pack(
                anchor='w', pady=(0, 4))
        ...
        ctk.CTkLabel(
            ...
            text_color='#9ba8b7', font=get_ui_font(9),
        ...
        for i, key in enumerate(['章节数', '总行数', '总字数']):
            ctk.CTkLabel(stat_frame, text=f"{key}:",
                         font=get_ui_font(9), width=50).grid(
                row=0, column=i * 2, padx=(0, 4))
            lbl = ctk.CTkLabel(stat_frame, text="-", width=80,
        ...
        # 章节目录（ttk.Treeview）
        catalog_inner = self._section(self.frame, "章节目录",
                                      fill='both', expand=True, pady=(4, 4))
        columns = ("num", "title", "lines", "start", "end", "chars")
        self.tree = ttk.Treeview(catalog_inner, columns=columns,
        ...
        vscroll = ctk.CTkScrollbar(catalog_inner, command=self.tree.yview)
        self.tree.configure(yscrollcommand=vscroll.set)
        self.tree.pack(side='left', fill='both', expand=True)
        vscroll.pack(side='right', fill='y')

        # 封面选项
        cover_inner = self._section(self.frame, "封面选项", fill='x', pady=(0, 4))
        cover_row = ctk.CTkFrame(cover_inner, fg_color='transparent')
        cover_row.pack(fill='x')

        self.cover_var = tk.IntVar(value=config.get('cover_choice', 4))
        for val, text in [(1, "无封面"), (2, "本地图片"),
                          (3, "图片 URL"), (4, "自动搜索（推荐）")]:
            ctk.CTkRadioButton(cover_row, text=text, value=val,
                               variable=self.cover_var,
                               command=self._on_cover_change,
                               font=get_ui_font(10)).pack(side='left', padx=(0, 12))

        self.cover_input_frame = ctk.CTkFrame(cover_inner, fg_color='transparent')
        self.cover_input_frame.pack(fill='x', pady=(4, 0))
        self.cover_input_var = tk.StringVar()
        self.cover_input_entry = ctk.CTkEntry(
            self.cover_input_frame, textvariable=self.cover_input_var,
            font=get_ui_font(10))
        self.cover_input_entry.pack(side='left', fill='x', expand=True, padx=(0, 6))
        self.cover_browse_btn = ctk.CTkButton(
            self.cover_input_frame, text="浏览...", width=80,
            font=get_ui_font(10), command=self._browse_cover)
    ...
    def _on_cover_change(self):
        ...
        if choice == 2:
            self.cover_input_entry.configure(state='normal')
            self.cover_browse_btn.configure(state='normal')
        elif choice == 3:
            self.cover_input_entry.configure(state='normal')
            self.cover_browse_btn.configure(state='disabled')
        else:
            self.cover_input_entry.configure(state='disabled')
            self.cover_browse_btn.configure(state='disabled')
    ...
    def _browse_cover(self):
        path = filedialog.askopenfilename(
            title="选择封面图片",
            filetypes=[
                ("JPEG 图片", "*.jpg"), ("JPEG 图片", "*.jpeg"),
                ("PNG 图片", "*.png"), ("GIF 图片", "*.gif"),
                ("WEBP 图片", "*.webp"), ("所有文件", "*"),
            ]
        )
        if path:
    ...
    # 封面候选选择流程
    # ------------------------------------------------------------------
    def _start_auto_cover_search(self, path, title, author):
        if not title:
        ...
        print("=" * 60)
        print(f"🔍 自动搜索封面候选: 《{title}》" + (f" / {author}" if author else ""))
        print("=" * 60)
    ...
    def _on_candidates_ready(self, candidates, path, title, author):
        ...
        if not candidates:
            print("⚠️ 未找到任何封面候选")
            if messagebox.askyesno("未找到封面",
                                    f"未能为《{title}》找到封面候选。\n是否继续生成（无封面）？"):
                self._run_generate_pipeline(path, title, author, None, False, None, 4)
            return
    ...
    def _on_cover_downloaded(self, local_path, path, title, author):
        if local_path:
    ...
    def _on_cover_download_failed(self, err, path, title, author):
        print(f"❌ 封面下载异常: {err}")
        if messagebox.askyesno("封面下载失败",
                                f"封面下载异常: {err}\n是否继续生成（无封面）？"):
            self._run_generate_pipeline(path, title, author, None, False, None, 4)
    ...
    def _on_candidates_search_failed(self, err, path, title, author):
        print(f"❌ 封面搜索异常: {err}")
        if messagebox.askyesno("封面搜索失败",
                                f"封面搜索异常: {err}\n是否继续生成（无封面）？"):
            self._run_generate_pipeline(path, title, author, None, False, None, 4)
        else:
            self.generate_btn.configure(state='normal')
    ...
    # 生成 EPUB 主流程
    # ------------------------------------------------------------------
    def _run_generate_pipeline(self, path, title, author,
                                cover_image, auto_search, cover_url, choice):
        ...
        print(f"   封面: {['无', '本地图片', 'URL', '自动搜索'][choice-1]}")
        print(f"   模式: {'转换 + 生成 EPUB' if need_convert else '直接生成 EPUB'}")
        print("-" * 60)
        ...
        def _on_complete(result):
            ...
            if epub_path:
                ...
                if need_convert:
                    self.path_var.set(final_txt)
                    self._load_catalog(final_txt)
            else:
