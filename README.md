...
**生成 EPUB**：封面选项包括无封面 / 本地图片 / 图片 URL / 自动搜索（含封面候选预览弹窗）。选择文件后自动分析章节，内嵌**章节编辑**区域——可修改章节标题/编号、上移/下移、删除章节，保存为新文件后自动切换生成路径。生成的 EPUB 结构：文案简介 → 目录页 → 各章节正文。
...
当相邻章节的人名/地名实体**完全不重叠**（连续 3 章）时，自动停止并输出诊断：
